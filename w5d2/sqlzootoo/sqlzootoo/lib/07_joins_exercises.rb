...
def ford_supporting_films
  ...
  JOIN castings on movies.id = castings.movie_id
  WHERE castings.actor_id = (
    SELECT id FROM actors WHERE name = 'Harrison Ford' AND castings.ord != 1
  )
  SQL
end
...
def andrews_films_and_leads
  ...
  #select actors.name from actors join castings Where castings.ord = 1
  execute(<<-SQL)
...
def films_by_cast_size
  ...
  ORDER BY COUNT(castings.actor_id) DESC, title ASC
  SQL
end
